...
def generate_tsp_common(image, fname, idx2ary, ary2idx, quadrant):
    ...
    for idx in range(1, len(idx2ary)+1):
        ...
        i1j0 = (src[0]+1, src[1])
        if i1j0 in ary2idx:
            f.write('%d %d %d\n' % (idx, ary2idx[i1j0], 1000*(1+color_cost(src, i1j0, image))))

        i0j1 = (src[0], src[1]+1)
        if i0j1 in ary2idx:
            f.write('%d %d %d\n' % (idx, ary2idx[i0j1], 1000*(1+color_cost(src, i0j1, image))))

        i1j1 = (src[0]+1, src[1]+1)
        if i1j1 in ary2idx:
            f.write('%d %d %d\n' % (idx, ary2idx[i1j1], 1000*(q2+color_cost(src, i1j1, image))))

        i1j_1 = (src[0]+1, src[1]-1)
        if i1j_1 in ary2idx:
            f.write('%d %d %d\n' % (idx, ary2idx[i1j_1], 1000*(q2+color_cost(src, i1j_1, image))))

        if config[quadrant]['j2'] == True:
            i0j2 = (src[0], src[1]+2)
            if i0j2 in ary2idx:
                f.write('%d %d %d\n' % (idx, ary2idx[i0j2], 1000*(q2+color_cost(src, i0j2, image))))

            i1j2 = (src[0]+1, src[1]+2)
            if i1j2 in ary2idx:
                f.write('%d %d %d\n' % (idx, ary2idx[i1j2], 1000*(q3+color_cost(src, i1j2, image))))

            i1j_2 = (src[0]+1, src[1]-2)
            if i1j_2 in ary2idx:
                f.write('%d %d %d\n' % (idx, ary2idx[i1j_2], 1000*(q3+color_cost(src, i1j_2, image))))

        if config[quadrant]['i2'] == True:
            i2j0 = (src[0]+2, src[1])
            if i2j0 in ary2idx:
                f.write('%d %d %d\n' % (idx, ary2idx[i2j0], 1000*(q2+color_cost(src, i2j0, image))))

            i2j1 = (src[0]+2, src[1]+1)
            if i2j1 in ary2idx:
                f.write('%d %d %d\n' % (idx, ary2idx[i2j1], 1000*(q3+color_cost(src, i2j1, image))))

            i2j_1 = (src[0]+2, src[1]-1)
            if i2j_1 in ary2idx:
                f.write('%d %d %d\n' % (idx, ary2idx[i2j_1], 1000*(q3+color_cost(src, i2j_1, image))))
...
def generate_tsp_upper_right(image, fname, quadrant):
    idx2ary, ary2idx = santatools.get_idxary_map(quadrant)
